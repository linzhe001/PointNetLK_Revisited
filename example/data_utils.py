...
# 体素下采样函数，替代open3d的voxel_down_sample
def voxel_down_sample(points, voxel_size):
    ...
    dims = np.maximum(dims, 1)  # 确保至少有一个体素
    
    # 计算每个点所在的体素索引
    voxel_indices = np.floor((points - min_bound) / voxel_size).astype(int)
    
    # 创建体素的唯一标识
    voxel_hash = voxel_indices[:, 0] + voxel_indices[:, 1] * dims[0] + voxel_indices[:, 2] * dims[0] * dims[1]
    
    # 找到每个唯一体素中的点
    unique_voxels, inverse, counts = np.unique(voxel_hash, return_inverse=True, return_counts=True)
    
    # 对于每个体素选择代表点（这里简单取体素中所有点的平均值）
    downsampled_points = []
    for voxel_id in unique_voxels:
        points_in_voxel = points[voxel_hash == voxel_id]
        # 取体素中心点
        voxel_center = np.mean(points_in_voxel, axis=0)
...
def load_3dmatch_batch_data(p0_fi, p1_fi, voxel_ratio):
    ...
    # 使用我们自己的体素下采样函数
    p0_downsampled = voxel_down_sample(p0, voxel_ratio)
    p1_downsampled = voxel_down_sample(p1, voxel_ratio)
...
class PointRegistration_fixed_perturbation(torch.utils.data.Dataset):
    def __init__(self, dataset, rigid_transform, sigma=0.00, clip=0.00, max_samples=None):
        ...
        if max_samples is not None and max_samples > 0:
             actual_len = min(dataset_len, max_samples)
             print(f"PointRegistration: 限制样本数量为 {actual_len}")
        else:
             actual_len = dataset_len
             print(f"PointRegistration: 使用全部 {actual_len} 个样本")

        self.actual_len = actual_len # 存储实际使用的样本数量

        # 加载转换矩阵，确保数量不超过实际样本数
        try:
             # 加载所有转换
             self.transf_ = load_pose(rigid_transform) 
             num_poses_available = len(self.transf_)
             print(f"从 {rigid_transform} 加载了 {num_poses_available} 个转换姿态")
             
             # 检查转换数量是否足够
             if num_poses_available < self.actual_len:
                  print(f"警告: 转换文件中的姿态 ({num_poses_available}) 少于请求的样本数 ({self.actual_len})。将使用所有可用的转换。")
                  self.actual_len = num_poses_available
                  # 如果数据集本身也需要被截断（虽然ModelNet构造函数已经处理了，但为了安全）
                  if hasattr(self.dataset, 'samples') and len(self.dataset.samples) > self.actual_len:
                       self.dataset.samples = self.dataset.samples[:self.actual_len]


             # 使用实际需要的转换数量，并打乱顺序
             list_order = torch.randperm(num_poses_available)
             self.transf = self.transf_[list_order[:self.actual_len]] # 只取需要的数量
             print(f"最终使用 {len(self.transf)} 个转换姿态进行测试")

        except Exception as e:
             print(f"加载或处理转换文件 {rigid_transform} 时出错: {e}")
             # 如果加载失败，可能需要采取备用策略或抛出错误
             raise IOError(f"无法加载姿态文件: {rigid_transform}") from e
    ...
    def __getitem__(self, index):
        ...
        if index >= self.actual_len:
            raise IndexError(f"索引 {index} 超出范围，实际样本数为 {self.actual_len}")
            ...
            try:
                 ...
                 if len(row) == 6:
                     poses.append(row)
                      ...
                      print(f"警告: 跳过格式不正确的行于 {trans_pth}: {row}")
            except ValueError as e:
                 print(f"警告: 跳过无法转换为浮点数的行于 {trans_pth}: {row} - {e}")

        init_gt = np.array(poses)
    print('从 {} 加载的 init_trans 形状为 {}'.format(trans_pth, init_gt.shape))
...
class ModelNet(Globset):
    ...
    def __init__(self, dataset_path, train=1, transform=None, classinfo=None, max_samples=None):
        loader = offread
        if train > 0:
            pattern = 'train/*.off'
        elif train == 0:
        ...
        # 如果指定了 max_samples，则截断样本列表
        if max_samples is not None and max_samples > 0 and len(self.samples) > max_samples:
            print(f"ModelNet: 限制样本数量从 {len(self.samples)} 到 {max_samples}")
            # 可以选择随机抽样或直接截断，这里我们直接截断
...
class C3VDset4tracking(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 从self.transf.igt获取变换矩阵 - 直接返回[4,4]形状
        igt = self.transf.igt
        
        # 不要使用unsqueeze(0)，让DataLoader处理批处理
...
class C3VDset4testing(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset, num_points, pose_file=None):
        ...
        if pose_file and os.path.exists(pose_file):
            ...
            self.transformations = load_pose(pose_file) # self.transformations 存储的是6D扰动向量
    ...
    def __getitem__(self, idx):
        try:
            ...
            if source.shape[0] < 100 or target.shape[0] < 100:
                raise ValueError(f"点云索引 {idx} 点数少于100")
            ...
            if scale < 1e-10:
                raise ValueError(f"点云索引 {idx} 缩放太小: {scale}")
            ...
            if self.transformations is not None and idx < len(self.transformations):
                perturbation_np = self.transformations[idx] # 6D numpy array
                # 将扰动向量转换为 [1, 6] 的张量，并移动到与点云相同的设备
                ...
                # 从6D扰动向量计算4x4的 ground truth 变换矩阵
                igt = utils.exp(perturbation_vec).squeeze(0) # igt 现在是 [4, 4] 的张量
                
                if idx < 3:  # 只为前几个样本打印日志
                    # 打印原始的6D扰动向量，与之前日志保持一致
                    print(f"样本 {idx}: 应用扰动向量 {perturbation_np.tolist()}")
                    # 打印计算得到的4x4变换矩阵
                    print(f"  - 计算得到的变换矩阵 (igt 4x4):\n{igt}")
            ...
            # 返回(模板，源，变换矩阵)
            # target作为模板(不动的)，source_norm作为源(被扰动的需要变换的)
            return target_norm, source_norm, igt # igt 现在是 4x4 矩阵
            
        except Exception as e:
            print(f"处理点云索引 {idx} 时出错: {str(e)}")
            # 建议在此处添加更详细的错误追踪，例如：
            # import traceback
            # traceback.print_exc()
            raise
...
class SinglePairDataset(torch.utils.data.Dataset):
    """单对点云测试数据集，支持指定扰动矩阵"""
    def __init__(self, source_file, target_file, num_points, transform_matrix=None, perturbation=None):
        self.source_file = source_file
        self.target_file = target_file
        self.num_points = num_points
        self.transform_matrix = transform_matrix  # 4x4矩阵
        self.perturbation = perturbation  # 6维向量 [wx,wy,wz,vx,vy,vz]
        self.resampler = Resampler(num_points)
        
        print(f"创建单对点云数据集:")
        print(f"  - 源点云: {source_file}")
        print(f"  - 目标点云: {target_file}")
        if transform_matrix is not None:
            print(f"  - 使用指定的4x4变换矩阵")
        if perturbation is not None:
            print(f"  - 使用指定的6维扰动向量: {perturbation}")
    ...
    def __getitem__(self, idx):
        # 读取源和目标点云
        source = plyread(self.source_file)
        target = plyread(self.target_file)
        ...
        if self.transform_matrix is not None:
            # 直接使用提供的变换矩阵
            igt = self.transform_matrix
        elif self.perturbation is not None:
            # 应用扰动向量生成变换矩阵
            _, igt = self.apply_transform(source_norm, torch.from_numpy(self.perturbation).unsqueeze(0))
        else:
            # 默认为单位矩阵
            igt = torch.eye(4)
        
        print(f"点云处理完成:")
        print(f"  - 源点云形状: {source_norm.shape}")
        print(f"  - 目标点云形状: {target_norm.shape}")
        print(f"  - 变换矩阵:\n{igt}")
